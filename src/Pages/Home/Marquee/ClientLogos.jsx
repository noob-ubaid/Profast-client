import React from "react";
import Marquee from "react-fast-marquee";

// import your logos
import amazon from "../../../assets/brands/amazon.png";
import google from "../../../assets/brands/google.png";
import casio from "../../../assets/brands/casio.png";
import moonstar from "../../../assets/brands/moonstar.png";
import start from "../../../assets/brands/start.png";
import randstad from "../../../assets/brands/randstad.png";
import people from "../../../assets/brands/people.png";

const logos = [amazon, google, people, casio, moonstar, start, randstad];

const ClientLogos = () => {
  return (
    <section className="py-10 mt-6 md:mt-10">
      <div className="w-full">
        <h2 className="text-2xl text-black font-bold text-center mb-12">
          Trusted by Leading Brands
        </h2>

        <Marquee pauseOnHover speed={80} gradient={false}>
          {logos.map((logo, idx) => (
            <div key={idx} className="md:mx-24 mx-8 flex items-center">
              <img
                src={logo}
                alt={`Client Logo ${idx + 1}`}
                className="h-8 object-contain"
              />
            </div>
          ))}
        </Marquee>
      </div>
    </section>
  );
};

export default ClientLogos;
